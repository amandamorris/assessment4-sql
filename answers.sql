...
SELECT brands.name, brands.founded FROM brands FULL OUTER JOIN models USING (brand_id) WHERE brands.brand_id NOT IN (SELECT brand_id FROM models);
...
SELECT models.name, models.year FROM models JOIN brands USING (brand_id) WHERE models.year IN (SELECT founded FROM brands);